...
alter table public.faturas
  drop constraint if exists faturas_arquivo_hash_format;

alter table public.faturas
  add constraint faturas_arquivo_hash_format
  check (
    arquivo_hash is null
    or arquivo_hash ~ '^[0-9a-f]{64}$'
  );

create unique index if not exists faturas_user_arquivo_hash_unique
  on public.faturas (user_id, arquivo_hash)
